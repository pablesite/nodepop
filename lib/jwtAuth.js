...
module.exports = function () {
    ...
    return function (req, res, next) {
        ...
        if (!token) {
            const err = new Error('No token provided. Please go to /apiv1/login in order to obtain a new token'); //esto se puede internacionalizar, en cualquier sitio de hecho
            err.status = 401;
            next(err);
            return;
        }
        ...
        jwt.verify(token, process.env.JWT_SECRET, (err, payload) => {
            if (err) {
                const err = new Error('Invalid signature. Please go to /apiv1/login in order to obtain a new token');
                err.status = 401;
                next(err);
                return;
            }
